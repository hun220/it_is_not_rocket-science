...
// Összevont verzió 2.0        //
///////////////////////////////

//EEPROM a mérés számának tárolása
#include <EEPROM.h>
...
void setup()
{
  //START I2C communication > Teensy - MPU6050 - BMP180
  Wire.begin();
  //START SERIAL communication > for USB debug
  Serial.begin(38400);
...
//======================================
  ...
  Serial.print("SD kartya inditasa");
  pinMode(10, OUTPUT);                  //ChipSelect pin a sd kártyához

  if (!SD.begin(SD_CS_PIN)) {
    Serial.println("SD kartya nem indul!");
    return;
  }

  //Sikeres inicalizálás!
  Serial.println(" |OK| Inicializálás oke! |OK| ");

 // Ha a fájl sikeresen megynyílt, akkor írunk bele.
  adat = SD.open("data.txt", FILE_WRITE);
  if (adat) {

    Serial.println(" |NEW DATA|");
    adat.print(EEPROM.read(0));
    adat.println(" ====================================================== ");

    // írás után bezárjuk a fájlt, elkrülve ezzel az esetleges összeakadást más programrészekkel.
    adat.close();
    Serial.println(" |OK| Adatok sikeresen rogzitve |OK| ");
}
else {
      // Ha nem sikerül a fájlba írni jelezzük.
      Serial.println(" |HIBA| Nem sikerült a fajlba irni! |HIBA| ");
     }
...
void fajliras()
{
  ...
  if (adat) {
    ...
    Serial.print(" | Adatok rogzitese az SD -kartyara! | ");

    adat.print(meres_sorszam);
    adat.print(",");
    adat.print(millis() - first_time);
    adat.print(",");
    adat.print(x_tengely_acc);
    adat.print(",");
    adat.print(alt);
    adat.println();
...
//  // fájl újra nyitása olvasásra.
//  adat = SD.open("data.txt");
//  if (adat) {
//    Serial.println("data.txt:");
//
//    // Kiolvasni a data.txt teljes tartalmát.
//    while (adat.available()) {
//      Serial.write(adat.read());
//    }
//    // close the file:
//    adat.close();
//  } else {
//    // Ha a fájlt nem tudjuk megnyitni kiratjuk.
//    Serial.println(" |HIBA| Nem sikerult olvasasra megnyitni a fajlt! |HIBA| ");
//  }
...
/*
   ...
   // Serial.print(1000*ay); Serial.print(";");
    ...
    x_tengely_acc = 1000*az;
    alt = bmp.readAltitude(kezdeti_nyomas-10);
